...
//  プロパティ（Black Out / Fill Light）をキャラ単位でまとめて制御する。
//
//  Dissolve 制御は EasyShaderCore の DissolveController へ移管した（マテリアルへ
//  直接書く経路を Controller 1 点に一本化するため。入れ替わり演出は
//  DissolveSwapController）。本コンポーネントは Dissolve を扱わない。
//
//  設計:
...
namespace Origuma.EasyPBR.URP
{
    ...
    public class DollLiveDirector : MonoBehaviour
    {
        ...
        {
            bool any = overrideBlackOut || overrideFill;
        ...
        {
            overrideBlackOut = overrideFill = false;
        }
    }
}
