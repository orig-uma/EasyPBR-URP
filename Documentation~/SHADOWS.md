...
左右（R/G）は **X Axis Tilt**（ベイク設定・度、既定 0）でスイープ面を顔 Up 方向へ倒し、「やや上から差す光」を前提に焼ける。水平スイープのままだと顎下〜首の境界が実際のライトとずれ、モデルによっては首まわりの影が不自然になる場合の調整。ベイク時のみの調整で、記録される値の意味（境界角度）は変わらない。
...
**顔 Forward 軸上の退化**: このウェイトは Forward 軸まわりの方位だけで決まるため、光が真正面・真後ろを通る瞬間は左右・上下成分が同時に 0 になり方位が定まらない。そのまま正規化すると R↔G が段差で入れ替わるので、横成分の長さ（`|sin(光と Forward のなす角)|`）が軸から約 14.5° 以内では 4 チャンネルの平均へフェードして連続にしている。この錐の内側は全面が光るか全面が陰るかのどちらかなので、絵の作りには影響しない。
